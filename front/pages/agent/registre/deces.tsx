   ...
   const router = useRouter()
    ...
    return<>
        ...
        <main className={styles.main}>
            <div>
                <h1>Declarations de Deces</h1>
                <p style={{"marginBottom":"3%", "marginTop":"2%", "fontSize":"17px"}}>Visionner les differentes declarations</p>
            </div>
            <ul>
                {data.map((item)=>(
                    <li key={item.id}>
                        <div className={styles.liste} onClick={()=>router.push(`data-deces/${item.numeroDoc}`)}>
                            <div>
...
export const getServerSideProps: GetServerSideProps = async() =>{    
    const res = await fetch('http://localhost:8000/api/liste-deces')
